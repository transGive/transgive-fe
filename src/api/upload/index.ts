...
export const uploadImages = (files: File[]) => {
    ...
    const estimatedTimeout = Math.max(
        120000,
        Math.ceil(totalSizeMB * 20000) + 60000,
    ); // Min 2min, 20s per MB + 1min buffer
    console.log(
        `Upload timeout set to ${estimatedTimeout}ms for ${totalSizeMB.toFixed(2)}MB`,
    );
