        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
            ...
            if (originalRequest.url?.includes("/auth/refresh")) {
                ...
                try {
                    ...
                    console.error(
                        "Error clearing access token in localStorage:",
                        clearError,
                    );
                }
                return Promise.reject(error);
            ...
            try {
                ...
                try {
                    ...
                    console.error(
                        "Error updating access token in localStorage:",
                        updateError,
                    );
                }
        ...
        if (error.response) {
            ...
            if (
                ...
                return Promise.reject(
                    new Error(
                        "Upload timeout: File is too large or connection is too slow. Please try again or upload smaller files.",
                    ),
                );
            }
            return Promise.reject(
